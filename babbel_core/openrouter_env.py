...
def _thread_messages_to_openrouter(thread_dict: dict) -> list:
    ...
    tail = history[-(max_pairs*2):] if max_pairs > 0 else history
    for m in tail:
        role = m.get("role", "user")
        content = m.get("content", "")
        if isinstance(content, list):
            parts = [_prepare_content(p) for p in content]
...
def use_chat(thread_dict: dict) -> dict:
    ...
    headers = {
        ...
        "X-Title": "Babbel Conversation Test",
        "HTTP-Referer": "http://localhost/",
    }
    payload = {
        "model": model,
        "messages": _thread_messages_to_openrouter(thread_dict),
    ...
    msg_obj = SimpleNamespace(role="assistant", content=content)
    return {"messages": [msg_obj]}
